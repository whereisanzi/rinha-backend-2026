...
const CONNECT_MAX_RETRIES: u32 = 200;
...
pub fn run() -> std::io::Result<()> {
    ...
    let mut handoffs: Vec<Box<Handoff>> = (0..MAX_HANDOFFS).map(|_| Box::new(Handoff::new())).collect();
...
fn handle_accept(
    ...
    *rr = rr.wrapping_add(1);
    ...
    push_sendmsg(ring, slot, h.generation, upstreams[idx].ctrl_fd, &mut h.msg as *mut _);
}
...
fn connect_ctrl(u: &Upstream) -> std::io::Result<RawFd> {
    let mut tries = 0u32;
    loop {
        ...
        tries += 1;
        if tries >= CONNECT_MAX_RETRIES {
            return Err(std::io::Error::new(
                err.kind(),
                format!("ctrl connect to {} failed: {}", u.path, err),
            ));
        }
